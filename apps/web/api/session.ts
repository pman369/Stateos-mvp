...
const SessionRequestSchema = z.object({
  ...
  currentState: StateVectorSchema,
  requiredState: StateVectorSchema.optional(),
  objective: z.string().trim().min(1).max(300).optional(),
  afterState: StateVectorSchema.optional(),
...
export async function handleSession(input: unknown, context: SessionContext = { reflections: [] }) {
  ...
  switch (request.action) {
    ...
    case "select": {
      ...
      const match = protocolEngine.match(context.intent, request.currentState, demoProtocols());
    ...
    case "execute": {
      ...
      const completedStepIds = context.protocol.steps.map((item) => item.id);
    ...
    case "complete": {
      ...
      return { step: "adapt", sessionId: request.sessionId, transition, reflection: aggregate, patterns, baseline, persisted: false, persistence: "Client local history is authoritative for this MVP; server persistence is intentionally not enabled yet." };
    }
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    return Response.json(await handleSession(body));
  } catch (error) {
    const message = error instanceof z.ZodError ? error.issues.map((issue) => `${issue.path.join(".")}: ${issue.message}`).join("; ") : error instanceof Error ? error.message : "Invalid session request.";
...
function demoProtocols(): Protocol[] {
  return [{
    id: "focus-transition", name: "Focus Transition", objective: "Move toward clear, present, intentional execution.",
    targetDimensions: ["attention", "clarity", "presence", "intentionality"], minimumState: { recovery: 0.2 },
    steps: [
